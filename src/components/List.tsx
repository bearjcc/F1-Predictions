	...
	render() {
		return (
			<div className="flag-container">
				<img
					className="flag"
					src={require(`../img/flags/${this.props.country}.svg`)}
...
export class Position extends React.Component<{ position: number }> {
	...
	render() {
		return <div className="position">{this.positions[this.props.position]}</div>;
	}
}
		...
		return (
			<div className="number">
				<img
					...
					src={require(`../img/numbers/${this.props.obj.name}.png`)}
					alt={this.props.obj.number.toString()}
				/>
			</div>
		);
	}
	...
	render() {
		return (
			<div className="2names">
				{this.props.smallFirst ? (
					<div className="smallName">{this.props.smallName}</div>
				) : (
					<div className="largeName">{this.props.largeName}</div>
				)}
				{this.props.smallFirst ? (
					<div className="largeName">{this.props.largeName}</div>
				) : (
